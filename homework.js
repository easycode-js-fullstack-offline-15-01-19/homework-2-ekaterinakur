...
let a1 = 0 || 'string';  // string  - || возвращает второй операнд, так как первый преобразовывается в false
let a2 = 1 && 'string';  // string  - && возвращает второй операнд, так как первый преобразовывается в true
let a3 = null || 25;  // 25  - || возвращает второй операнд, так как первый преобразовывается в false
let a4 = null && 25;  // null  - && возвращает первый операнд, так как он преобразовывается в false
let a5 = null || 0 || 35;  // 35  - || возвращает последний операнд, так как первые два преобразовываются в false
let a6 = null && 0 && 35;  // null  - && возвращает первый операнд, так как он преобразовывается в false
...
f === 0 ? f = 1 
	: f < 0 ? f = 'less then zero'
		: f *= 10;

//  3 --- Написать условие если возраст машины больше 5 лет то нужно вывести в консоль сообщение 'Need Repair' и свойство needRepair в объекте car изменить на true; иначе изменить на false.

let car1 = { name: 'Lexus', age: 10, create: 2008, needRepair: false }

car1.age > 5 ? (console.log('Need Repair'), car1.needRepair = true)
	: car1.needRepair = false;